...
aws iam put-role-policy \
  ...
  --policy-document file://5g-pipeline-trigger-lambda-role.json
...
aws iam put-role-policy \
  ...
  --policy-document file://5g-sagemaker-execution-role.json
...
aws iam put-role-policy \
  ...
  --policy-document file://5g-fastapi-lambda-role.json
```
...
```bash
# Upload ONLY the raw garbo CSVs (exclude cleaned/scaled/map outputs)
aws s3 sync backend/data/ s3://5g-dashboard-raw-data/ \
  --exclude "map_data.csv" \
  --exclude "cleaned_5g_data.csv" \
  --exclude "cleaned_5g_scaled.csv"

# Upload cleaned/scaled data to the cleaned bucket
aws s3 cp backend/data/cleaned_5g_data.csv s3://5g-dashboard-cleaned-data/cleaned_5g_data.csv
aws s3 cp backend/data/cleaned_5g_scaled.csv s3://5g-dashboard-cleaned-data/cleaned_5g_scaled.csv
...
**`backend/requirements.lambda.txt`** — Slim deps for the API only (no training libs):
...
> **Why a separate file?** The full `requirements.txt` includes xgboost, catboost, scikit-learn,
> etc. for notebook training. XGBoost 3.x requires cmake to build from source — which the Lambda
> base image doesn't have — and including all ML libs would bloat the image to ~2 GB with slow
> cold starts. The API only needs fastapi + pandas + mangum + resend.
...
# Copy Lambda-only requirements and install (excludes training deps like xgboost/catboost)
...
# Build and tag (--provenance=false required for Lambda to accept the image)
cd backend
...
> **Note:** CloudFront is created later in Phase 5. Use `"*"` for CORS origins now so you can
> test the API immediately. Phase 5.6 will lock CORS down to the actual CloudFront domain.

```bash
# Create the HTTP API (CORS uses "*" temporarily — locked down in Phase 5.6)
...
```json
{
  ...
  "Origins": {
    ...
    "Items": [
      {
        "Id": "S3-frontend",
        "DomainName": "5g-dashboard-frontend.s3.us-east-1.amazonaws.com",
        "S3OriginConfig": { "OriginAccessIdentity": "" },
        "OriginAccessControlId": "OAC_ID_PLACEHOLDER"
      },
      {
        "Id": "APIGateway-backend",
        "DomainName": "API_ID_PLACEHOLDER.execute-api.us-east-1.amazonaws.com",
        "CustomOriginConfig": {
          "HTTPPort": 80,
          "HTTPSPort": 443,
...
Once CloudFront is live, lock down CORS so only your CloudFront domain can call the API.
There are **two things** to update:

**A) API Gateway CORS** — restricts which browser origins the gateway accepts:

```bash
# Create cors-locked.json:
# {
#   "AllowOrigins": ["https://d33m3uevv39v9p.cloudfront.net"],
#   "AllowMethods": ["GET", "POST", "OPTIONS"],
#   "AllowHeaders": ["Content-Type", "Authorization"],
#   "MaxAge": 3600
# }
...
aws apigatewayv2 update-api \
  --api-id lhrvjvhw3g \
  --cors-configuration file://cors-locked.json
```

**B) FastAPI CORS middleware** — add the CloudFront domain to `allow_origins` in
`backend/app/main.py` so FastAPI also responds with the correct `Access-Control-Allow-Origin`
header:

```python
# In backend/app/main.py, add the CloudFront domain to allow_origins:
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
        "http://localhost:5173",
        "http://localhost:8080",
        "http://localhost:4173",
        "https://d33m3uevv39v9p.cloudfront.net",  # ← Production
    ],
...
After updating `main.py`, **rebuild and redeploy** the Lambda image:

```bash
cd backend
docker build -f Dockerfile.lambda -t 5g-dashboard-backend --provenance=false .
docker tag 5g-dashboard-backend:latest \
  677276113002.dkr.ecr.us-east-1.amazonaws.com/5g-dashboard-backend:latest
...
aws lambda update-function-code \
  --function-name 5g-dashboard-api \
  --image-uri 677276113002.dkr.ecr.us-east-1.amazonaws.com/5g-dashboard-backend:latest
```
