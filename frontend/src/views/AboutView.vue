...
const members: TeamMember[] = [
  ...
  },
  {
    ...
    school: 'Swinburne University of Technology',
    major: 'Bachelor of Computer Science (Artificial Intelligence)',
    bio: 'Detail-oriented data analyst with a strong foundation in exploratory analysis and statistical reasoning. Focused on transforming raw 5G telemetry into clean, model-ready datasets that drive accurate predictions and meaningful visual insights.',
    contributions: [
      'Performed exploratory data analysis across 138 raw 5G drive-test CSVs, identifying outliers, missing-value patterns, and distribution skews in throughput and latency columns',
      'Co-designed the feature engineering pipeline — selecting and normalising key predictors (RSRP, RSRQ, SNR, CQI) used by both the clustering and forecasting models',
      'Validated clustering output by cross-referencing quality labels against geographic signal-strength heatmaps to ensure spatial consistency',
      'Assisted in hyperparameter tuning for the XGBoost forecasting model, running grid-search experiments and logging metrics for comparison',
    ],
    avatarInitials: 'VTP',
  ...
  },
  {
    ...
    contributions: [
      'Architected and validated the end-to-end ML pipeline for 5G upload bitrate prediction — from data ingestion through evaluation reporting',
      'Trained and benchmarked five model families (Random Forest, XGBoost, LSTM, GRU, and Mamba-SSM), producing a comparative performance analysis on MAE, RMSE, and R² metrics',
      'Designed and implemented ensemble methods that fuse tabular (XGBoost) and sequential (LSTM/GRU) predictions, improving forecast accuracy over single-model baselines',
      'Conducted ablation studies on feature subsets and window sizes to identify optimal input configurations for the time-series models',
    ],
    avatarInitials: 'NTMH',
    avatarColor: 'bg-orange-500',
  },
  {
    ...
    school: 'Swinburne University of Technology',
    major: 'Bachelor of Computer Science (Artificial Intelligence)',
    bio: 'Analytical thinker who enjoys uncovering stories hidden in raw data. Skilled in data wrangling, quality assurance, and building reproducible analysis workflows that keep machine-learning pipelines reliable.',
    contributions: [
      'Led data-quality auditing — flagged and corrected invalid GPS coordinates (99999 sentinel values) and network-timeout entries across 138 CSVs before model training',
      'Built summary statistics reports and correlation matrices to guide feature selection for the KMeans clustering and XGBoost forecasting models',
      'Curated the final cleaned dataset (map_data.csv) used by the backend API, ensuring latitude/longitude precision and consistent quality-label encoding',
      'Supported model evaluation by generating confusion-matrix visualisations and per-cluster performance breakdowns for the clustering pipeline',
    ],
    avatarInitials: 'LMK',
